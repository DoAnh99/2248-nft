...
public class tutorials2 : Singleton<tutorials2>
{
    ...
    void Start()
    {
        ...
        blockClone2.colorBlock = new Color((((float)blockClone2.valueBlock * 1043) % 16383) / 16383, ((((float)blockClone2.idUnitBlock * 1053) % 16383) + 2567) / 16383, ((((float)blockClone2.valueBlock * 143) + 457) % 16383) / 16383, 1);
        ...
        blockClone.naturalPart = 4;
        blockClone.valueBlock = 4;
        ...
        blockClone1.naturalPart = 8;
        blockClone1.valueBlock = 8;
        blockClone1.idUnitBlock = 0;
        blockClone1.idColor = 2;
              ...
              lineRend.enabled = true;
                ...
                if (touch.phase == TouchPhase.Moved)
                {
                    ...
                    if (hit.collider != null)
                    {
                        ...
                        {                  
                          ...
                          //  if (DistancePos < 1.17 && DistancePos > 0.60)
                            if (DistancePos < 1.8 && DistancePos > 1.2)
                            {
                                Debug.Log("DistancePos" + DistancePos);
                                if (blocks.Count == 1)
                                {
                ...
                // if (blocks.Count > 0)
                {
                    ...
                    if (BestScoreCrr == 16)
                    {
                        ...
                        {
                            //start.Instance.PanelRewardAccessstoreParent.SetActive(true);
                            //start.Instance.PanelRewardAccessstore.SetActive(true);
                            start.Instance.Panel3.SetActive(false);
                            start.Instance.Panelstart.SetActive(true);
    ...
    public void removeRenderLineCurrent(int numberVertextRenderline)
    {
    ...
    public void moveHand()
    {
        ...
        moveTime = timer;
        handTutorial.transform.DOMove( point1.position, 0.2f).OnComplete(() => {
            handTutorial.transform.DOMove(point3.position, 1f);
        });
    }
