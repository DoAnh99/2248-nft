       ...
       // public DailyRewards dailyRewards;			// DailyReward Instance      
        ...
        private void OnInitialize(bool error, string errorMessage)
        {
            if (!error)
            {
                ...
                UpdateUI();
                canvas.gameObject.SetActive(!DataManager.Instance.UserData.Tutorial && (showWhenNotAvailable || (!showWhenNotAvailable && isRewardAvailable)));
